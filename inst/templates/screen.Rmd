...
Also defining some common variables for gene-level statistics.

```{r}
gene.ids <- SummarizedExperiment::rowData(se)[[<%= GENE_FIELD %>]]
all.genes <- sort(unique(gene.ids))
:BEGIN create-gene-annotation
extra.gene.data <- SummarizedExperiment::rowData(se)[match(all.genes, gene.ids),<%= ROW_DATA_FIELDS %>,drop=FALSE]
:END
:BEGIN create-fry-indices
gene.indices <- split(seq_along(v$genes$origin), gene.ids[v$genes$origin])
:END
```
:END
...
fry.res[is.na(fry.res$NumBarcodes)] <- 0L
:BEGIN gene-annotation
...
augere.core::saveResult(
    holm.res, 
    file.path(result.dir, paste0("holm-", length(all.holm.min))),
:BEGIN merge-metadata
